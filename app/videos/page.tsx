...
function VideosContent() {
    ...
    const fetchVideos = async () => {
        ...
        try {
            ...
            const resAuth = await fetch(`/api/get-account-token?email=${email}`);
            // Note: need to implement this endpoint or just fetch directly if I can query DB?
            // Actually, existing /api/get-stats returns ALL accounts. I can filter client-side or make a specific one.
            // Let's use get-stats and filter for now to save time creating new endpoint.
            ...
            // Set Token
            window.gapi.client.setToken({ access_token: account.access_token });
            ...
            const mappedVideos: VideoItem[] = items.map((item: any) => ({
                id: item.contentDetails.videoId, // Use contentDetails.videoId for playlist items
